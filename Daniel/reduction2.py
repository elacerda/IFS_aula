...
if __name__ == '__main__':
    ...
    iraf.gsflat(
        ...
        bias=rawdir + bias, fl_over='no', fl_vardq=vardq, fl_fulldq=vardq)
    iraf.gsflat(
        rawdir + flat_star, specflat='flat_star.fits', rawpath=rawdir,
        bias=rawdir + bias_star, fl_over='no', fl_vardq=vardq, fl_fulldq=vardq)   
    ...
    iraf.gsextract(
        'stgs' + star)
    iraf.gsstandard(
        'estgs' + star, sfile='std', sfunction='sens',
        caldir='onedstds$ctionewcal/', starname='l3218')   
    ...
    for i in gal:
        iraf.gsreduce(
            ...
            fl_fixpix='yes', flat='flat.fits', fl_over='no',
            fl_gscrrej='yes', fl_vardq=vardq, fl_fulldq=vardq)
        iraf.gstransform(
            'gs' + i, wavtraname='gs' + arc.replace('.fits', ''),
            fl_vardq=vardq)
        iraf.gsskysub(
            'tgs' + i, long_sample='800:900', fl_vardq=vardq)
        iraf.gscalibrate(
            'stgs' + i, fl_vardq=vardq)
